...
| 文件 | 职责 |
| --- | --- |
| `webpack-hook.js` | hook `webpackChunkheybox_chat.push`；注入设置侧栏；已安装插件列表按 `panel.id === plugin.id` 挂「设置」，钻取渲染对应 panel；官方 `force_update` 弹窗补回「取消」 |
...
模块 ID（1.56.0 / 1.57.0 / 1.57.1 **当前**相同，换版本可能变）对插件隐藏，只在 hook 内部使用：`42416` 设置菜单、`93509` UserConfig、`30570` EventBus、`16886` Vuex。插件走 `BHChat` 稳定面；必须碰官方模块时运行时探测，不要写「仅某某版本」。
...
| Phase 3 社区模板、多版本映射 | 1.56 / 1.57.x 暂共用同一套 ID，未做映射表；兼容约定见 [09-plugin-dev.md](./09-plugin-dev.md)「版本兼容」 |

## 关键 Patch 点（1.56.0 / 1.57.0 / 1.57.1，路径相同）
...
热更新可能覆盖 `webapp`。`main-bridge` 在窗口加载前跑 `ensurePatches`，缺标记就补回 html / preload / index.js。货架插件 `block-update` 阻断 `/chatroom/v2/settings/version/update/check`；main-bridge 用 session webRequest 取消该请求，IPC 只作下载兜底。框架本身不屏蔽检查更新，但会给官方 `force_update` 弹窗补回「取消」（`webpack-hook` 探测 `UpdateDialog`，不写死模块号），避免全新安装时被卡住进不了设置。
